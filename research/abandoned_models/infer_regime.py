...
def decode_current_regime(n_states: int = None, context_bars: int = None):
    ...
    context_bars = CONTEXT_BARS if context_bars is None else context_bars # This determines the past 2000 candles data , which we want to calculate the current HMM regime 

    artifact = load_artifact(n_states) # Load the production model artifact (HMM model, scaler, feature columns, etc.) for the specified number of states
    ...
    #The Goal : Create a clean feature table and mark which rows are safe for the HMM to use.
    ...
    context = feats.iloc[-context_bars:] if len(feats) > context_bars else feats ## This is for taking only the 2000 candles from the current 

    # We can only decode the regime for the most recent bar if it is valid (not touched by a zero-volume gap and not NaN). If the last bar is invalid, we cannot report a regime for it.
